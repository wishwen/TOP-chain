...
TEST_F(test_mem_leak, block_mem_leak_1_BENCH) {
    ...
    top::data::xblock_ptr_t genesis_block;
    top::data::xblock_ptr_t block;
    {
        ...
        genesis_block = tableblocks[0];
        block = tableblocks[1];
        ASSERT_TRUE(blockstore->store_block(mocktable, block.get()));
        EXPECT_EQ(block->get_refcount(), 1 + 1);

        // auto rb = blockstore->load_block_object(mocktable, 1, 0, 0);
        // EXPECT_EQ(block->get_refcount(), 1 + 1);  // blockstore cache ref 1, prev pointer ref 1
    }

    EXPECT_EQ(genesis_block->get_refcount(), 1);
    EXPECT_EQ(block->get_refcount(), 1);
}
